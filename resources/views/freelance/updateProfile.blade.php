        ...
        <div class="sixteen columns">
            <form action="{{ route('freelance.profile.post') }}" method="post">
                ...
                <div class="submit-page">
                    ...
                    <div class="form">
                        <h5>Update Name</h5>
                        <input class="search-field" name="name" type="text" required
                    ...
                    <div class="form">
                        ...
                        <input class="search-field" name="title" required type="text" placeholder="e.g. Web Developer"
                            @if (Auth::user()->userable->profession) value={{ Auth::user()->userable->profession }} @else {{ old('title') }} @endif>
                    </div>

                    <!-- Location -->
                    <div class="form">
                        <h5>Location</h5>
                        <input class="search-field" name="location" required type="text" placeholder="e.g. London, UK"
                            @if (empty(old('location'))) value={{ Auth::user()->userable->location }} @else {{ old('location') }} @endif>
                    </div>
                    <!-- amail -->
                    <div class="form">
                        <h5>Email</h5>
                        <input class="search-field" name="email" required type="text" placeholder="e.g. London, UK"
                            @if (empty(old('email'))) value={{ Auth::user()->email }} @else {{ old('email') }} @endif>
                    </div>

                    <!-- Salaries -->
                    <div class="form">
                        <h5>Salary</h5>
                        <input class="search-field" name="salaries" type="number" min="5" placeholder="30 ($)"
                            @if (empty(old('salaries'))) value={{ Auth::user()->userable->salaries }} @else {{ old('salaries') }} @endif>
                    </div>
                    {{-- <div class="form">
                        <h5>Email</h5>
                    ...
                    <div class="form">
                        ...
                        <textarea class="WYSIWYG" name="description" cols="40" rows="3" id="summary" spellcheck="true"
                            value="{{ old('description') }}" required>  @if (Auth::user()->userable->description)
{{ Auth::user()->userable->description }}
@else
{{ old('title') }}
@endif  </textarea>
                    </div>
                    ...
                    <button type="submit" class="button big margin-top-5">Preview <i
