              ...
              @auth
                    @if (Auth::user()->role_id != 2)
                       @if (empty($job->end_at))
                            ...
                            @if( freelance_jobs::Where('freelance_id',Auth::user()->userable->id)->Where('job_id',$job->id)->Where('is_hired',1)->first() )
                             <a href="#" class=" button" style="background-color:rgb(3, 70, 33)"> You had already enrolled </a>
                            @elseif( freelance_jobs::Where('freelance_id',Auth::user()->userable->id)->Where('job_id',$job->id)->first() )
